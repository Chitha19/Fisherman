	...
	public void paintComponent(Graphics g){
		...
		g.setColor(Color.BLACK);
   		...
   		for(int i=0;i<h.size();i++){
                    hook ba = h.get(i);
      		    g.drawImage(ba.imfire[ba.count%5].getImage(), ba.x,120,50,50, null);
      		    ba.move();
      		    ba.count++;
      		    if(ba.y<0){
      		    	h.remove(i);
		...
		g.drawImage(b.getImage(), 580, 360, 400, 400, this);
                ...
                g.drawImage(b.getImage(), b.x,120,604,369, this);
		if(b.x<-250){
                    b.x=this.getWidth()-300;
		}
		if(b.x>this.getWidth()-20){
                    b.x=-200;
		}
		for(int i=0;i<h.size();i++){
		    hook ba = h.get(i);
